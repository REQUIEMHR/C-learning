...
多态性意味着有多重形式。在面向对象变成范式中，多态往往表现为“一个接口，多个功能”。
...
在编译时，函数调用就已经解决，即函数绑定发生在编译时。C# 提供了两种静态多态性：函数重载和运算符重载。
...
以重定义或重载 C# 中内置的运算符。因此，程序员也可以使用用户自定义类型的运算符。重载运算符是具有特殊名称的函数，是通过关键字 operator 后跟运算符的符号来定义的。与其他函数一样，重载运算符有返回类型和参数列表。
...
| #define | 定义宏 |
| #undef | 取消已定义的宏 |
| #if | 如果指定的条件为真，则编译指定的代码块 |
| #else | #if 的替代方案 |
| #elif | 如果前面的 #if 条件不为真，编译指定的代码块 |
| #endif | 结束一个 #if-#else 条件编译块 |
| #error | 当遇到标准错误时，输出错误消息 |
| #warning | 当遇到编译器警告时，输出警告消息 |
| #region | 定义一个可展开或折叠的代码块 |
| #endregion | 结束一个 #region 块 |
| #line | 指定行号和文件名 |
| #pragma | 发出特殊指令到编译器 |
| #nullable | 启用或禁用对代码元素中的可空类型的支持 |

#### #define 指令和 #undef 指令

#define 指令创建符号常量。#undef 指令删除符号常量。
